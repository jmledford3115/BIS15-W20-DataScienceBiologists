...
homerange %>% 
  ggplot(aes(x=log10.hra)) +
  geom_histogram(aes(y = ..density..), fill = "palevioletred2", alpha = 0.8, color = "black")+
...
homerange <- homerange%>% 
    mutate(range_category = case_when(log10.hra <= 3.65 ~ "small",
                                      log10.hra > 3.65 & log10.mass <= 4.59 ~ "very small",
                                      log10.hra > 4.59 & log10.mass <= 6.02 ~ "medium",
                                      log10.mass >= 6.03 ~ "large"))
```
